...
impl Thread {
    ...
    pub fn get_thread(conn: &SqliteConnection, find_uri: &str) -> Result<String> {
        ...
        let thread_info = threads
            ...
            .load::<Thread>(conn).chain_err(|| ErrorKind::DBRead)?;
    ...
    pub fn create_thread<'t>(conn: &SqliteConnection, new_url: &'t str, new_title: Option<&'t str>) -> Result<bool> {
        ...
        let result = diesel::insert(&new_thread).into(threads::table)
            .execute(conn)
            .is_ok();
        Ok(result)
    }
